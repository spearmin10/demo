...
usage_exit() {
  echo "Usage: $0 [-n container-suffix] [-r] [-h]" 1>&2
  exit 1
}
...
if [ -z "${TSRAIN_CONTAINER_ID}" ]; then
  ...
  export RAINLOOP_DEFAULT_ADMIN_PASSWORD="tsradminPassword123$"
  export MAILBOX_PASSWORD="Password123$"

  CREDS_PATH="/var/opt/tsrain/services/${CONTAINER_NAME}/credentials.json"
  if [ ! -f ${CREDS_PATH} ]; then
    echo "{\"*\":{\"password\":\"${MAILBOX_PASSWORD}\"}}" > ${CREDS_PATH}
  fi
  
  TSRAIN_CONTAINER_ID=`docker run --rm --memory 128m --memory-swap 1g -d -p "25:25" -p "80:80" -p "143:143" -p "993:993" \
    --name "${CONTAINER_NAME}" \
